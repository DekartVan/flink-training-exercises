...
public class RidesAndFaresExercise extends ExerciseBase {
	...
	public static class EnrichmentFunction extends RichCoFlatMapFunction<TaxiRide, TaxiFare, Tuple2<TaxiRide, TaxiFare>> {
		...
		private ValueState<TaxiRide> ridesState;
		private ValueState<TaxiFare> faresState;

		@Override
		public void open(Configuration config) throws Exception {
			ridesState = getRuntimeContext().getState(new ValueStateDescriptor<>("saved rides", TaxiRide.class));
			faresState = getRuntimeContext().getState(new ValueStateDescriptor<>("saved fares", TaxiFare.class));
		}

		//Join
		@Override
		public void flatMap1(TaxiRide ride, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			TaxiFare fare = faresState.value();
			if (fare != null) {
				faresState.clear();
				out.collect(new Tuple2<>(ride, fare));
			} else {
				ridesState.update(ride);
			}
		}

		@Override
		public void flatMap2(TaxiFare fare, Collector<Tuple2<TaxiRide, TaxiFare>> out) throws Exception {
			TaxiRide ride = ridesState.value();
			if (ride != null) {
				ridesState.clear();
				out.collect(new Tuple2<>(ride, fare));
			} else {
				faresState.update(fare);
			}
		}
	}
}